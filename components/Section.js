...
const TOCSectionIntro = ({ options = {
  id, dataTitle, dataDesc, number
}}) => {
  return (
    <section className="layout full-bleed pt-24 pb-12 bg-gray-100">
      {options.number && 
        <span className="block font-semibold text-gray-500 mb-2 text-3xl">{options.number}</span>
      } 
      <h2 className="font-bold text-5xl text-gray-900 mb-8">
        <a href={`#${options.id}`}>{options.dataTitle}</a>
      </h2>
...
const Section = ({ children, className, options = {
  id: null,
  dataTitle: null,
  dataDesc: null,
  number: null,
  ...
  return (
    ...
    >
      {options.dataTitle && 
        <TOCSectionIntro options={options} />
      }
      { children }
    </section>
  );
